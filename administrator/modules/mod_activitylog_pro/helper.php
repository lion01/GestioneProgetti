...
class modActivityLogHelperHelper {
	...
	public function getUsers() {
		$query = 'SELECT DISTINCT ' . self::$db->nameQuote('ualog.user_id') . ',' 
									. self::$db->nameQuote('users.id') . ',' 
									. self::$db->nameQuote('users.name') . ','
									. self::$db->nameQuote('users.username') 
				. ' FROM  ' . self::$db->nameQuote('#__ualog') . ' AS  ualog '
				. ' LEFT JOIN ' . self::$db->nameQuote('#__users') . ' AS users '
	...
	public function getOptions() {
		$query = 'SELECT ' . self::$db->nameQuote('option') 
				. ' FROM  ' . self::$db->nameQuote('#__ualog')
				. ' GROUP BY ' . self::$db->nameQuote('option')
				. ' ORDER BY ' . self::$db->nameQuote('option') . ' ASC';
        self::$db->setQuery($query);
