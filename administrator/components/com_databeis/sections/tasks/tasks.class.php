...
class PFtasksClass extends PFobject
{
	...
	public function SaveTask()
	{
		...
		$campo1 = JRequest::getVar('campo1');
		$campo2 = JRequest::getVar('campo2');
		$campo3 = JRequest::getVar('campo3');
		$campo4 = JRequest::getVar('campo4');
		...
		$query = "INSERT INTO #__pf_tasks VALUES("
		       ...
		       . "\n ".$db->Quote($priority).",'$milestone', '$ordering', '$poddb', NULL, '$typology', ".$db->Quote($campo1).", ".$db->Quote($campo2).", ".$db->Quote($campo3).", ".$db->Quote($campo4)
		       . "\n )";
		       $db->setQuery($query);
		       $db->query();
		       $id = $db->insertid();
	...
	public function UpdateTask($id)
	{
		...
		$campo1db = JRequest::getVar('campo1');
		$campo2db = JRequest::getVar('campo2');
		$campo3db = JRequest::getVar('campo3');
		$campo4db = JRequest::getVar('campo4');
		...
		if ($progress == 100) {
			...
			$query = "UPDATE #__pf_tasks SET title = ".$db->quote($title).", fdate = ".$db->quote($now).", pod = '$poddb', content = ".$db->quote($content).","
			   ...
			   . "\n , campo1 = '$campo1db', campo2 = '$campo2db', campo3 = '$campo3db', campo4 = '$campo4db',"
		...
		while ($count > $i)	{
			...
			if (isset($file['name'])) {
				...
				if (!JFile::upload($file['tmp_name'], $filepath.DS.$prefix2.strtolower($file['name']))) {
					$i++;
					$e = true;
					$this->AddError(PFL_E_FILE_UPLOAD);	
					continue;
				}
...
PFformat::Logging($query);
				...
				if((int) $config->Get('attach_files', 'filemanager')) {
					if($this->save_attachments($id, 'file', $task)) {
						return true;
					}
	...
	public function save_attachments($id, $type, $task)
	{
		$db = PFdatabase::GetInstance();
