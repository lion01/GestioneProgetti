...
class adminPraiseUninstallation {
	...
	public function dropTables() {
		...
		$query = 'DROP TABLE ' . $db->nameQuote('#__adminpraise_menu');
		$db->setQuery($query);
		if ($db->query()) {
		...
		$query = 'DROP TABLE ' . $db->nameQuote('#__adminpraise_menu_types');
		$db->setQuery($query);
		if ($db->query()) {
	...
	public function uninstallTemplate() {
		...
		$query = 'SELECT extension_id FROM ' . $this->db->nameQuote('#__extensions')
	...
	public function uninstallPlugins() {
	  ...
	  */
		$installer = new JInstaller;
		$query = 'SELECT extension_id FROM ' . $this->db->nameQuote('#__extensions')
				. ' WHERE element = ' . $this->db->Quote('activitylog_pro');
		$this->db->setQuery($query);
	  ...
	  */
		$installer = new JInstaller;
		$query = 'SELECT extension_id FROM ' . $this->db->nameQuote('#__extensions')
				. ' WHERE element = ' . $this->db->Quote('adminpraise_autoeditor');
		$this->db->setQuery($query);
	  ...
	  */
		$installer = new JInstaller;
		$query = 'SELECT extension_id FROM ' . $this->db->nameQuote('#__extensions')
				. ' WHERE element = ' . $this->db->Quote('admin_menu')
				. ' AND folder = ' . $this->db->Quote('adminpraisesearch');
		$this->db->setQuery($query);
