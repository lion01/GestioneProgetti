...
class AdminpraiseHelper {
	...
	public function isTemplateEnabled() {
		$db = JFactory::getDBO();
		$query = 'SELECT template FROM ' . $db->nameQuote('#__template_styles')
	...
	public function turnAdminpraiseOn() {
		if (!self::isTemplateEnabled()) {
			$link = JRoute::_('index.php?option=com_adminpraise&view=settings&task=publishTemplate&' . JUtility::getToken() . '=1');
	...
	public function checkAdminMenuHealth() {
		...
		if(!$menuItems || !$menuTypes || !$menuModules) {
			...
			if(!$menuItems) {
				...
				$link = JRoute::_('index.php?option=com_adminpraise&view=menu');
			...
			if(!$menuTypes) {
				...
				$link = JRoute::_('index.php?option=com_adminpraise&view=menu&task=newMenuType');
			...
			$link = JRoute::_('index.php?option=com_adminpraise&view=menu&task=reset&'. JUtility::getToken() . '=1');
	...
	private function checkAdminMenuTypes() {
		$db = JFactory::getDBO();
		$query = 'SELECT count(id) AS count FROM ' . $db->nameQuote('#__adminpraise_menu_types');
		$db->setQuery($query);
	...
	private function checkAdminMenuModules() {
		$db = JFactory::getDBO();
		$query = 'SELECT count(id) AS count FROM ' . $db->nameQuote('#__modules')
